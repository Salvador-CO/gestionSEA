        ...
        <div class="col-xl-10 col-lg-10">
            ...
            <div class="card shadow-sm mb-4 border-start border-4 border-info animate__animated animate__fadeInDown">
                <div class="card-body d-flex align-items-center py-3">
                    <div class="icon-box me-3 bg-info bg-opacity-10 p-3 rounded-circle">
                        <i class="bi bi-shield-lock-fill fs-3 text-info"></i>
                    </div>
                    <div>
                        <h6 class="mb-1 fw-bold text-dark">Gestión de Inscripciones - {{ $user->centro ?? 'Administración Global' }}</h6>
                        <p class="small mb-0 text-muted">
                            <span class="badge bg-light text-dark border">Usuario: minúsculas</span>
                            <span class="badge bg-light text-dark border">Pass: 8+ car, Mayús, Num y Símbolo (*, ., -, #)</span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-md-7">
                    <div class="card shadow-lg h-100 animate__animated animate__fadeInLeft">
                        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                            <h5 class="mb-0 fw-bold"><i class="bi bi-person-plus-fill text-primary me-2"></i>Registro de Estudiante</h5>
                            <a href="{{ route('registro.usuarios') }}" class="btn btn-outline-primary btn-sm rounded-pill px-3">
                        ...
                        <div class="card-body p-4">
                            <div id="step1" class="mb-4">
                                <label class="form-label small fw-bold text-uppercase text-muted">1. Validar Correo Institucional</label>
                                <div class="input-group">
                                    <span class="input-group-text bg-white border-end-0"><i class="bi bi-envelope text-primary"></i></span>
                                    <input type="email" id="check_email" class="form-control order-start-0 shadow-none" placeholder="correo@ejemplo.com" value="{{ request('email') ?? old('email') }}">
                            ...
                            <form action="{{ route('registro.store') }}" method="POST" id="formRegistro" style="display:none;" onsubmit="return validarFormulario()" class="animate__animated animate__fadeIn">
                                @csrf
                                <div class="row g-3">
                                    <div class="col-12">
                                        <label class="form-label small fw-bold">CORREO DISPONIBLE</label>
                                        <input type="email" name="email" id="final_email" class="form-control bg-light fw-bold text-success border-success" readonly>
                                    </div>
                                    <div class="col-12">
                                        <label class="form-label small fw-bold">USUARIO / MATRICULA</label>
                                        <input type="text" name="username" id="username" class="form-control" placeholder="ejemplo: 26500289a" 
                                        value="{{ request('matricula') ?? old('username') }}"
                                        required>
                                        <div id="userError" class="text-danger tiny-text" style="display:none;"><i class="bi bi-x-circle"></i> Mínimo 4 caracteres (letras minúsculas y números).</div>
                                    </div>
                                    <div class="col-sm-6">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div id="passError" class="text-danger tiny-text" style="display:none;"><i class="bi bi-exclamation-triangle"></i> No cumple con las políticas de seguridad.</div>
                                    </div>
                                    <div class="col-sm-6">
                                    ...
                                    <div class="col-12 mt-4">
                                        <button type="submit" class="btn btn-success w-100 py-3 fw-bold btn-action shadow" style="background: var(--success-gradient); border:none;">
                ...
                <!-- COLUMNA ASIGNACIÓN DE CURSOS -->
                <div class="col-md-5">
                    <div id="sectionCursos" style="display:none;" class="card shadow-lg border-0 h-100 animate__animated animate__fadeInRight">
                        <div class="card-header bg-dark text-white py-3 d-flex justify-content-between align-items-center">
                            <h6 class="mb-0 fw-bold"><i class="bi bi-journal-plus me-2"></i>2. Asignar Cursos</h6>
                            <span id="badgeContador" class="badge bg-primary badge-contador rounded-pill">0 / 8</span>
                        </div>
                        <div class="card-body p-4">
                            <!-- Datos del estudiante cargado -->
                            ...
                            <div id="cursoSelector">
                                ...
                                <div class="text-center my-3" id="spinnerGrupos" style="display:none;">
                                    ...
                                    <span class="small ms-2 text-muted">Buscando grupos espere puede tardar...</span>
                                </div>
                                ...
                                <button id="btnAddCourse" class="btn btn-primary w-100 py-2 fw-bold btn-action mb-4">
                                    ...
                                    <span id="btnLoading" style="display:none;"><span class="spinner-border spinner-border-sm"></span> Procesando...</span>
                                </button>
                            </div>

                            <!-- Listado y Finalización -->
                            ...
                            <ul class="list-group list-group-flush border rounded-3 mb-4 bg-white shadow-sm" id="ulCursos">
                                <li id="liVacio" class="list-group-item text-center text-muted py-3 small">Ningún curso asignado aún</li>
                            </ul>

                            <!-- Botón de finalizar dinámico -->
                            <div id="finalizarWrapper" style="display:none;" class="animate__animated animate__fadeInUp">
                                <p class="tiny-text text-center text-success mb-2"><i class="bi bi-info-circle"></i> Puede finalizar ahora o agregar más cursos.</p>
                                <button id="btnFinalizarTodo" class="btn btn-dark w-100 py-3 fw-bold btn-action shadow" onclick="confirmarSalida()">
                    ...
                    <div id="emptyCursos" class="card shadow-sm h-100 border-0 bg-light d-flex align-items-center justify-content-center p-5 text-center text-muted">
                        <i class="bi bi-arrow-left-circle display-4 mb-3 opacity-25"></i>
                        <p class="small fw-bold">Valide el correo para habilitar la inscripción a cursos.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        .then(data => {
            ...
            if(data.exists) {
                if(data.acceso_denegado) {
                    ...
                    // Doble candado: Guardamos de forma estricta los IDs que Moodle reporta que ya están cursados
    ...
    fetch(`{{ route('registro.obtenerCursos') }}?userId=${userId}`)
        ...
        .then(cursos => {
            ...
            cursos.forEach(c => { 
                ...
                // Candado JavaScript absoluto: Comprueba que el ID del curso no esté mapeado en la lista de pre-inscritos
    ...
    fetch(`/registro/obtener-grupos/${courseId}?centro=${encodeURIComponent(centro)}`)
        ...
        .then(grupos => {
            ...
            if(Array.isArray(grupos) && grupos.length > 0) {
                ...
                grupos.forEach(g => { 
                    // Renderizado del grupo con el contador compacto de participantes y nombre del Asesor a cargo
...
document.getElementById('btnAddCourse').addEventListener('click', function() {
    ...
    .then(data => {
        ...
        if(data.success) {
            ...
            // Remover el curso seleccionado del select para evitar re-inscripción inmediata
...
@if(session('nuevo_usuario_id'))
    iniciarFlujoCursos("{{ session('nuevo_usuario_id') }}", "{{ session('nuevo_usuario_nombre') }}", "{{ session('nuevo_usuario_centro') }}");
@endif
